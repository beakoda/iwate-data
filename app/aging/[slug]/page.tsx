...
import { MUNIS, PREF, muniBySlug, censusAt, agingRate, youthRate, workingRate, fmt, fmtSigned, pct, rank, LATEST_CENSUS, PREV_CENSUS } from '@/lib/data';
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const ages = [
    { label: '15歳未満', now: c.age_0_14, prev: c0.age_0_14 },
    { label: '15〜64歳', now: c.age_15_64, prev: c0.age_15_64 },
    { label: '65歳以上', now: c.age_65_, prev: c0.age_65_ },
  ];
  return (
    <>
      <Breadcrumb items={[{ name: '高齢化率・年齢構成', href: '/aging/' }, { name: m.name }]} />
      <DatasetJsonLd name={title} description={sentence} path={`/aging/${m.slug}/`}
        keywords={[m.name, '高齢化率', '年齢構成', '人口ピラミッド', '国勢調査', '岩手県']} temporal={`${PREV_CENSUS}/${LATEST_CENSUS}`} sourceKeys={['census']} />
      <h1>{title}</h1>
      ...
      <h2>年齢3区分別人口（{PREV_CENSUS}年 → {LATEST_CENSUS}年）</h2>
      <div className="table-wrap">
        <table>
          <thead><tr><th>区分</th><th>{PREV_CENSUS}年</th><th>{LATEST_CENSUS}年</th><th>増減</th><th>増減率</th><th>{LATEST_CENSUS}年の構成比</th></tr></thead>
          <tbody>
            {ages.map(a => { const d = a.now - a.prev; const denom = c.age_0_14 + c.age_15_64 + c.age_65_; return (
              <tr key={a.label}><td>{a.label}</td><td>{fmt(a.prev)}</td><td>{fmt(a.now)}</td>
                <td className={d < 0 ? 'neg' : d > 0 ? 'pos' : ''}>{fmtSigned(d)}</td>
                <td className={d < 0 ? 'neg' : d > 0 ? 'pos' : ''}>{fmtSigned(pct(a.now, a.prev), '%')}</td>
                <td>{(a.now / denom * 100).toFixed(1)}%</td></tr>); })}
            <tr className="hl"><td>総人口（年齢不詳を含む）</td><td>{fmt(c0.total)}</td><td>{fmt(c.total)}</td><td>{fmtSigned(c.total - c0.total)}</td><td>{fmtSigned(pct(c.total, c0.total), '%')}</td><td>100%</td></tr>
          </tbody>
        </table>
      </div>

      <LineChart title={`${m.name}と岩手県の高齢化率（${PREV_CENSUS}〜${LATEST_CENSUS}年）`} unit="%"
        series={[
          { label: m.name, points: [{ x: PREV_CENSUS, y: me.prevAging }, { x: LATEST_CENSUS, y: me.aging }] },
          { label: '岩手県', points: [{ x: PREV_CENSUS, y: agingRate(censusAt(PREF.code, PREV_CENSUS)) }, { x: LATEST_CENSUS, y: prefAging }], color: '#949494' },
        ]} />
