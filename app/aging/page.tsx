...
const pF = censusAt(PREF.code, FIRST_CENSUS)!;
...
export default function Page() {
  const rows = MUNIS.map(m => {
    const c = censusAt(m.code, LATEST_CENSUS)!, c0 = censusAt(m.code, PREV_CENSUS)!, cF = censusAt(m.code, FIRST_CENSUS)!;
    return { m, c, cF, aging: agingRate(c), prev: agingRate(c0), first: agingRate(cF), youth: youthRate(c), work: workingRate(c) };
  });
  ...
  return (
    <>
      ...
      <div className="table-wrap">
        <table>
          <thead><tr><th>順位</th><th>市町村</th><th>高齢化率</th><th>{PREV_CENSUS}年</th><th>{FIRST_CENSUS}年</th><th>変化</th><th>年少人口割合</th><th>生産年齢人口割合</th><th>平均年齢</th><th>人口</th><th>人口密度</th></tr></thead>
          <tbody>
            <tr className="hl"><td>—</td><td>岩手県</td><td>{fmt(prefAging)}%</td><td>{fmt(prefPrev)}%</td><td>{fmt(agingRate(pF))}%</td><td>{fmtSigned(Math.round(((prefAging ?? 0) - (prefPrev ?? 0)) * 10) / 10)}pt</td><td>{fmt(youthRate(p))}%</td><td>{fmt(workingRate(p))}%</td><td>{fmt(p.avg_age != null ? Math.round(p.avg_age * 10) / 10 : null)}歳</td><td>{fmt(p.total)}</td><td>{fmt(p.density)}</td></tr>
            {by.map(x => { const d = x.aging != null && x.prev != null ? Math.round((x.aging - x.prev) * 10) / 10 : null; return (
              <tr key={x.m.code}><td>{r.get(x) ?? '—'}</td><td><Link href={`/aging/${x.m.slug}/`}>{x.m.name}</Link></td>
                <td>{fmt(x.aging)}%</td><td>{fmt(x.prev)}%</td><td>{fmt(x.first)}%</td>
