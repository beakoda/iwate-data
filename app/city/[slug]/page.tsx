...
import { INDUSTRIES, MUNIS, PREF, muniBySlug, econAt, popAt, dentalAt, per100k, fmt, fmtSigned, pct, rank, LATEST_DENTAL, LATEST_POP } from '@/lib/data';
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <DatasetJsonLd name={title} description={sentence} path={`/city/${m.slug}/`} keywords={[m.name, '統計', '人口', '事業所数', '産業', '歯科診療所', '岩手県']} temporal={`2009/${LATEST_POP}`} sourceKeys={['population', 'econ2021', 'econ2016', 'dental']} />
      <h1>{title}</h1>
      ...
      <SourceBox keys={['population', 'econ2021', 'econ2016', 'dental']} extra={['特化係数＝当該市町村の産業別事業所構成比 ÷ 岩手県の産業別事業所構成比（事業所5以上の産業のみ表示）。']} />
    </>
  );
}
