...
SOURCES = {
 ...
 'econ2016': {'name':'総務省・経済産業省「平成28年経済センサス‐活動調査」事業所に関する集計 産業横断的集計 第8表（産業別民営事業所数・従業者数－都道府県、市区町村）岩手県', 'url':'https://www.e-stat.go.jp/stat-search/files?page=1&toukei=00200553&year=20160', 'note':'2016年6月1日現在。'},
    ...
    'crime': {'name':'岩手県警察「オープンデータ（街頭犯罪等の発生状況）」', 'url':'https://www.pref.iwate.jp/kenkei/koho/opendata/3000711.html', 'note':'岩手県警が公開する事件1件ごとの発生記録（発生地の市区町村コード・町丁目、発生年月日、発生場所、被害者属性）を、本サイトが市町村×年×手口で集計したもの。対象は自転車盗・車上ねらい・部品ねらい・自動販売機ねらい・自動車盗・オートバイ盗・ひったくりの7手口。年は発生年月日（始期）の年。刑法犯認知件数の全体ではなく、この7手口に限った件数である点に注意。／利用条件は岩手県警のウェブサイト利用規約により公共データ利用規約（第1.0版・PDL1.0）。出典表記は「犯罪オープンデータ」（岩手県警察）https://www.pref.iwate.jp/kenkei/koho/opendata/3000711.html を加工して作成。加工したのは本サイトであり、岩手県警察が作成したものではない。'},
...
def load_shofuku():
    """障害福祉サービス等情報公表システムの事業所個票を市町村×サービス種別で集計。e-Stat 由来ではない。"""
    out, svc = {}, set()
    for r in load_csvs('shofuku_offices.csv'):
        code = LEGACY.get(r['code'], r['code'])
        n, u = int(r['offices']), int(r['with_url'])
        assert u <= n, ('with_url > offices', code, r['service'], n, u)
        d = out.setdefault(code, {}).setdefault(r['service'], {'offices': 0, 'with_url': 0})
        d['offices'] += n; d['with_url'] += u
        svc.add(r['service'])
    for d in out.values():
        d['_total'] = {'offices': sum(v['offices'] for k, v in d.items() if not k.startswith('_')),
                       'with_url': sum(v['with_url'] for k, v in d.items() if not k.startswith('_'))}
    return out, sorted(svc)
...
def load_kaigo():
    ...
    out, svc = {}, set()
    for r in load_csvs('kaigo_offices.csv'):
        code = LEGACY.get(r['code'], r['code'])
        assert r['snapshot'] in KAIGO_SNAPS, ('unknown snapshot', r['snapshot'])
        d = out.setdefault(code, {}).setdefault(r['snapshot'], {})
        e = d.setdefault(r['service'], {'offices': 0, 'capacity': 0})
        e['offices'] += int(r['offices']); e['capacity'] += int(r['capacity'])
        svc.add(r['service'])
    for ss in out.values():
        for d in ss.values():
            d['_total'] = {'offices': sum(v['offices'] for k, v in d.items() if not k.startswith('_')),
                           'capacity': sum(v['capacity'] for k, v in d.items() if not k.startswith('_'))}
    return out, sorted(svc)
...
def build():
    ...
    gn = sum(sf[m['code']]['_total']['offices'] for m in munis)
    gu = sum(sf[m['code']]['_total']['with_url'] for m in munis)
    assert (gn, gu) == (1881, 1086), ('shofuku', (gn, gu), (1881, 1086))
    ...
    KAIGO_TOTAL = {'2024-12': 2581, '2026-06': 2535}
    for snap in KAIGO_SNAPS:
        got = sum(kai[m['code']].get(snap, {}).get('_total', {}).get('offices', 0) for m in munis)
        assert got == KAIGO_TOTAL[snap], ('kaigo', snap, got, KAIGO_TOTAL[snap])
