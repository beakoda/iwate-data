...
import {
  ...
  schoolCodeAt, SCHOOL_KINDS, SCHOOL_ASOF,
} from '@/lib/data';
...
export const FAMILIES: Record<string, Family> = {
  ...
  shofuku: { label: '障害福祉サービス事業所', years: [Number(SHOFUKU_ASOF.slice(0, 4))], yearLabel: '年',
    at: (code: string) => {
      const row: Record<string, any> = { 時点: SHOFUKU_ASOF };
      for (const t of SHOFUKU_SERVICES) {
        const c = shofukuAt(code, t);
        row[t] = c?.offices ?? 0; row[t + '_HP公表'] = c?.with_url ?? 0;
      }
      const all = shofukuAt(code);
      row['合計'] = all?.offices ?? 0; row['合計_HP公表'] = all?.with_url ?? 0;
      return row;
    },
    cols: [['時点', '時点'], ...SHOFUKU_SERVICES.flatMap(t => [[t, t] as [string, string], [t + '_HP公表', t + '_HP公表'] as [string, string]]), ['合計', '合計'], ['合計_HP公表', '合計_HP公表']] },
  school_code: { label: '学校（学校コード・現存校）', years: [Number(SCHOOL_ASOF.slice(0, 4))], yearLabel: '年',
    at: (code: string) => {
      const row: Record<string, any> = { 時点: SCHOOL_ASOF };
      for (const k of SCHOOL_KINDS) row[k] = schoolCodeAt(code, k);
      row['合計'] = schoolCodeAt(code);
      return row;
    },
    cols: [['時点', '時点'], ...SCHOOL_KINDS.map(k => [k, k] as [string, string]), ['合計', '合計']] },
