...
- ビルド: **1770ページ**生成成功（国勢調査＋建築着工＋人口動態＋世帯＋病院・医師＋介護施設＋学校＋所得製造業＋ごみ＋完全失業率＋最終学歴＋農家＋街頭犯罪＋介護サービス事業所）。主要数値は raw CSV と突き合わせ検算済み
...
### ページ構成（sitemap 1154件＋CSV＋埋め込み33本）
...
`raw/crime_2016_2025.csv` と `raw/kaigo_offices.csv` は e-Stat API ではなく、配布元のCSVを直接落として集計したもの。**`scripts/fetch_estat.py` も月次 cron（`scripts/update.sh`）もこの2つを更新しない。** 更新は手作業。
...
- **取得できる環境**: 日本の政府・自治体サイトはコンテナからも Cowork の Linux VM からも egress で 403 になる。**VPS（rocky@os3-306）と、ユーザーPCのChromeだけが到達できる**。Chrome から取る場合は「ページ内で fetch → Shift_JIS デコード → 集計 → 結果を DOM に書き出して読み取る」方式が有効（全国22万件のような巨大CSVを持ち出さずに済む）
- **検算**: `build_data.py` の assert に総件数をハードコードしてある（crime 4,885件／kaigo 2,581・2,535事業所）。**データを更新したらこの数字も更新すること。** 数字を消して assert を無効化しないこと
...
- **`/crime/` と `/kaigo/` は月次 cron の対象外**（e-Stat 由来ではないため `fetch_estat.py` が見ていない）。放っておくと静かに古くなる。更新手順は §3「非 e-Stat データの取り方」
- **MCP のカタログは `crime` だけ追加済み、`kaigo` は未対応**。`mcp/src/catalog.ts` は `key[code][year]` の形しか扱えないが、`kaigo` は `kaigo[code][時点][サービス種別]` と1段深いため。MCPで出すならカタログ側にネスト対応を足すこと
