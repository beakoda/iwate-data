...
### AI推薦調査（/ai/、2026-09-24 追加）

`lib/aiSurvey.ts` に手入力の集計値を持つ独自調査ページ。ChatGPT（GPT-5.4・Web検索有効、DataForSEO の LLM Responses API 経由）に「◯◯市でおすすめの◯◯」と聞き、挙がった事業者ごとに「AIが根拠として引用した情報源」を分類したもの。**事業者名はサイトに載せない**（集計のみ。名前つきの元データは社内保管 `work/internal/岩手歯科_AI推薦調査_2026-09.csv`）。
- 月次 cron の対象外。更新するときは同じ質問文で再測定し、`AI_MORIOKA` / `AI_DENTAL_CITIES` を書き換える（1回あたり約0.07ドル）
